<?php
  ...
  if($id_klinis_duodenum || $id_klinis) {
    if($id_klinis_duodenum) {
      $query_id = mysqli_query($conn, "SELECT * FROM data_klinis_duodenum WHERE id_klinis_duodenum='$id_klinis_duodenum'");
      $row = mysqli_fetch_assoc($query_id);
    } else {
      $query_id = mysqli_query($conn, "SELECT * FROM data_klinis_duodenum WHERE id_klinis_duodenum='$id_klinis'");
      $row = mysqli_fetch_assoc($query_id);
    }
...
<div class="container-content">
  <div class="containerTab" style="background:white">
    ...
    <h2>Klinis Duodenum-Jejunum-lleum</h2>
        ...
        <div class="col-sm-10">
          <select id="inputState" class="form-control" name="keluhan" value="<?php echo $keluhan; ?>" >
            <?php
              ...
              $data_keluhan = mysqli_query($conn, "SELECT keluhan FROM data_klinis_duodenum WHERE dk_duodenum_id_pasien = '$id_pasien'");
              while ($row = mysqli_fetch_array($data_keluhan)) {
                echo "<option selected value='".$row['keluhan']."'>".$row['keluhan']."</option>";
              }

              $all_keluhan = mysqli_query($conn, "SELECT keluhan FROM data_klinis_duodenum");
        ...
        <div class="col-sm-10">
          <select id="inputState" class="form-control" name="fam_history" value="<?php echo $fam_history; ?>" > 
            <?php
              ...
              $data_fhis = mysqli_query($conn, "SELECT fam_history FROM data_klinis_duodenum WHERE dk_duodenum_id_pasien = '$id_pasien'");
              while ($row = mysqli_fetch_array($data_fhis)) {
                echo "<option selected value='".$row['fam_history']."'>".$row['fam_history']."</option>";
              }

              $all_fhis= mysqli_query($conn, "SELECT fam_history FROM data_klinis_duodenum");
        ...
        <div class="col-sm-10">
          <select id="inputState" class="form-control" name="lokasi" value="<?php echo $lokasi; ?>" >
            <?php
              ...
              $data_loc = mysqli_query($conn, "SELECT lokasi FROM data_klinis_duodenum WHERE dk_duodenum_id_pasien = '$id_pasien'");
              while ($row = mysqli_fetch_array($data_loc)) {
                echo "<option selected value='".$row['lokasi']."'>".$row['lokasi']."</option>";
              }

              $all_loc = mysqli_query($conn, "SELECT lokasi FROM data_klinis_duodenum");
