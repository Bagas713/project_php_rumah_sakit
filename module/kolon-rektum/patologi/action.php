<?php
    ...
    $datas = mysqli_query($conn, "SELECT id_patologi_kolon FROM patologi_kolon WHERE dp_kolon_id_pasien = '$id_pasien'");
    while($dta = mysqli_fetch_assoc($datas)) {
        $id_patologi_kolonn = $dta['id_patologi_kolon'];
    }
    ...
    if($button == "Save") {
        mysqli_query($conn, "INSERT INTO patologi_kolon (tumor, node, metastasis, lokasi_metastasis, no_patologi_biopsi, tanggal_biopsi, jenis_patologi_biopsi, no_patologi_operasi_definitif, jenis_patologi_operasi_definitif, grade_histopatologi, reseksi, batas_reseksi_proksimal, batas_reseksi_distal, lvi, invasi_perineural, catatan_temuan_operasi, dp_kolon_id_pasien, dp_kolon_nama) VALUES ('$tumor', '$node', '$metastasis', '$lokasi_metastasis', '$no_patologi_biopsi', '$tanggal_biopsi', '$jenis_patologi_biopsi' , '$no_patologi_operasi_definitif', '$jenis_patologi_operasi_definitif', '$grade_histopatologi', '$reseksi', '$batas_reseksi_proksimal', '$batas_reseksi_distal', '$lvi', '$invasi_perineural', '$catatan_temuan_operasi', '$id_pasien', '$type_ill' )"); 
        
    } else if($button == "Update") {
    // $id_klinis_esofaguss = $_GET['id_klinis_esofagus'];
    mysqli_query($conn, "UPDATE patologi_kolon SET tumor = '$tumor',
                                                node = '$node',
                                                ...
                                                catatan_temuan_operasi = '$catatan_temuan_operasi' WHERE id_patologi_kolon = '$id_patologi_kolonn'");
    }

    $data = mysqli_query($conn, "SELECT id_patologi_kolon FROM patologi_kolon WHERE dp_kolon_id_pasien = '$id_pasien'");
    while($dta = mysqli_fetch_assoc($data)) {
        $id_patologi_kolon = $dta['id_patologi_kolon'];
    }

    header("location:".BASE_URL."index.php?page=module/$type_ill/patologi/form&id_pasien=$id_pasien&type_ill=$type_ill&id_patologi_kolon=$id_patologi_kolon");
?>